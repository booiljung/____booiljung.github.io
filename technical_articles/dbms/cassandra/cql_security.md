원문: [Apache: Cassandra: Security](http://cassandra.apache.org/doc/latest/cql/security.html)
...
Only a client with with the `CREATE` permission on the database roles resource may issue `CREATE ROLE` requests (see the [relevant section](http://cassandra.apache.org/doc/latest/cql/security.html#cql-permissions) below), unless the client is a `SUPERUSER`. Role management in Cassandra is pluggable and custom implementations may support only a subset of the listed options.
...
- To modify properties of a role, the client must be granted `ALTER` [permission](http://cassandra.apache.org/doc/latest/cql/security.html#cql-permissions) on that role
...
`DROP ROLE` requires the client to have `DROP` [permission](http://cassandra.apache.org/doc/latest/cql/security.html#cql-permissions) on the role in question. In addition, client may not `DROP` the role with which it identified at login. Finally, only a client with `SUPERUSER` status may `DROP` another `SUPERUSER` role.
